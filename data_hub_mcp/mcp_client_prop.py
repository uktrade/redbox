...
async def call_companies(client, company_name: str, page: int = 0, page_size: int = 10):
    async with client:
        result = await client.call_tool(
            "company_details", {"company_name": company_name, "page": page, "page_size": page_size}
        )
        log.logger.info(result)
...
def run_examples(client):
    ...
    asyncio.run(call_companies(client, "VERTEX", 0, 2))
    asyncio.run(call_companies(client, "VERTEX NIMBUS INDUSTRIES", 0, 2))
    asyncio.run(call_companies(client, "vertex nimbus industries", 0, 2))
