...
# def get_company_details(company_name: str, page_size: int = 10, page: int = 0) -> CompanySearchResult:
#     session = get_session()

#     companies: list[Company] = []
#     result_type: MCPToolResultType = MCPToolResultType.NONE
#     try:
#         single_match = session.query(Company).filter(Company.name == company_name).one_or_none()

#         if single_match is None:
#             companies = session.query(Company).filter(Company.name.ilike("%" + company_name + "%")).all()
#             result_type = MCPToolResultType.MULTI_SIM_MATCH if len(companies) > 0 else MCPToolResultType.
#         else:
#             result_type = MCPToolResultType.SINGLE_EXACT_MATCH
#             companies = [single_match]

#     except exc.MultipleResultsFound as e:
#         companies = session.query(Company).filter(Company.name == company_name).all()

#     if single_match is not None:
#         company_id = single_match.id
