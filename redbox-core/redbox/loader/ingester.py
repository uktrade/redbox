...
def get_elasticsearch_store_without_embeddings(es, es_index_name: str):
    ...
    return OpenSearchVectorSearch(
        ...
        opensearch_url=env.elastic.collection_endpoint,
        #embedding_function=get_embeddings(env),
        embedding_function=FakeEmbeddings(size=env.embedding_backend_vector_size) #set embedding size to match bedrock model amazon.titan-embed-text-v2:0 default embedding size
    )
