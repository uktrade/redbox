...
NEW_ROUTE_CITATION_PROMPT = (
    """Use citations to back up your answer when available. Return your response in the following format: {format_instructions}.
Example response:
If citations are available: {{"answer_process": "Using the title 'Steps Taken', outline the processes you took to produce the answer within {steps_max_token} words or less, and highlight any processes that were unsuccessful in answering the user question", "answer": "your complete answer here including any 'ref_N' citation markers inline as required in plain text", "citations": [list_of_citations]}}.
- Each citation must be shown in the answer using a unique identifier in the format "ref_N". Number each quote sequentially starting from ref_1, then ref_2, ref_3, and so on.
Do not repeat citations or citation identifiers across sources or documents. Do not include citation markers that do not exist.

If no citations are available or needed, return an empty array for citations like this: {{"answer_process": "Using the title 'Steps Taken', outline the processes you took to produce the answer within {steps_max_token} words or less, and highlight any processes that were unsuccessful in answering the user question", "answer": "your complete answer here with no citation markers", "citations": []}}.
Do not provide citation from your own knowledge.\n"""
    + MAX_TOKEN_PROMPT
    + """\nAssistant:<haiku>"""
)
...
NEW_ROUTE_RETRIEVAL_SYSTEM_PROMPT = """Answer user question using the provided context. Using the title 'Steps Taken', outline the processes you took to produce the answer within {steps_max_token} words or less, and highlight any processes that were unsuccessful in answering the user question"""
