...
import sqlite3
import time
from typing import Annotated, Callable, Iterable, Literal, Union, List, Tuple
import duckdb
...
def build_query_tabular_knowledge_base_tool() -> "Tool":
    """
    Returns a tool that takes a SQL query generated by the agent and executes it against
    tabular files (CSV, Excel) referenced in the agent prompt metadata.

    The agent is responsible for:
      - Selecting which URI(s) to query
      - Generating the SQL query based on the document_schema
    """

    @tool(response_format="content_and_artifact")
    def _query_tabular_knowledge_base(
        sql_query: str,
        tabular_metadata: Annotated[List[dict], "Agent-provided Tabular Metadata"],
    ) -> Tuple[str, List[Document]]:
        """
        Executes a pre-generated SQL query against tabular files described in
        tabular_metadata (agent prompt). Returns results as a list of Document objects.

        Args:
            sql_query (str): SQL query string generated by agent based on schema.
            tabular_metadata (list[dict]): Metadata about available tabular files.
        ...
        Returns:
            Tuple[str, List[Document]]:
                - A string summarizing the query results
                - A list of Document objects with page_content and metadata
        """
        documents: List[Document] = []
        result_text = ""

        # Execute SQL against each file in DuckDB
        for file_meta in tabular_metadata:
            uri = file_meta["metadata.uri"]
            try:
                ...
                if uri.lower().endswith(".csv"):
                    con.execute(f"CREATE TABLE {file_meta['metadata.name']} AS SELECT * FROM read_csv_auto('{uri}')")
                elif uri.lower().endswith((".xls", ".xlsx")):
                    con.execute(f"CREATE TABLE {file_meta['metadata.name']} AS SELECT * FROM read_excel('{uri}')")
                else:
                ...
                table_text = "\n".join([str(dict(zip(columns, row))) for row in query_result])
                result_text += f"\nResults from {uri}:\n{table_text}\n"

                # Wrap each row as a Document
                for row in query_result:
                    ...
                    doc = Document(
                        ...
                        metadata={
                            "uri": uri,
                            "table_name": file_meta["metadata.name"],
                        },
                    )
...
# def build_query_tabular_knowledge_base_tool() -> Tool:
#     """
#     Builds a tool that executes a pre-generated SQL query on the tabular knowledge base
#     already ingested into the state. The agent is expected to generate a valid SQL query
#     that matches the structure of state.knowledge_tabular_files.
#     """
...
#     ) -> tuple[str, list[Document]]:
#         """
#         Executes a SQL query on the tabular knowledge base stored in state.knowledge_tabular_files
#         and returns matching rows as a list of Documents along with a formatted string.

#         Args:
#             sql_query (str): Pre-generated SQL query from the agent, must match the document_schema
#                              of file in <Tabular_Knowledge_Base_Metadata>.
#             state (RedboxState): The current chain state containing ingested tabular files.

#         Returns:
#             tuple[str, list[Document]]: Formatted result string and list of matching Documents.
#         """
#         import duckdb
#         from redbox.models.chain import Document

#         # Flatten all tabular data into one DuckDB in-memory table
#         con = duckdb.connect(database=":memory:")
#         table_counter = 0
#         for group in state.knowledge_tabular_files.groups.values():
#             for table_name, doc in group.items():
#                 # doc.page_content is expected to be list of dicts
#                 con.register(f"{table_name}", doc.page_content)
#                 table_counter += 1

#         if table_counter == 0:
#             return "No tabular data found in the state.", []

#         # Execute the pre-generated query
#         try:
#             result_df = con.execute(sql_query).fetchdf()
#         except Exception as e:
#             return f"SQL execution error: {e}", []

#         # Convert DuckDB result into Documents
#         result_docs = [
#             Document(
#                 page_content=row.to_dict(),  # Each row as a dict
#                 metadata={"source": "knowledge_tabular_files"}
#             )
...
#         return result_str, result_docs

#     return _query_tabular_knowledge_base

# def build_query_tabular_knowledge_base_tool(
#     table_field: str = "knowledge_tabular_files",
# ) -> "Tool":
#     """
#     Tool that executes a pre-generated SQL query on tabular knowledge base stored in state,
#     using DuckDB.

#     The SQL query should be provided by a worker agent or stored in state.generated_sql_query.
#     Returns a DataFrame of results and the source documents used.
#     """

#     @tool(response_format="content_and_artifact")
#     def _query_tabular_knowledge_base(
#         sql_query: str | None = None,
#         state: Annotated[RedboxState, InjectedState] = None,
#     ) -> Tuple[pd.DataFrame, List[Document]]:
#         """
#         Executes a SQL query on tabular documents in the state using DuckDB.

#         If `sql_query` is None, the tool will attempt to use
#         `state.generated_sql_query`. Each document in `state.knowledge_tabular_files`
#         is registered as a DuckDB table using its `metadata.table_name` (or a generated
#         name if missing).

#         Args:
#             sql_query (str | None): The SQL query to execute. Should reference table
#                 names present in the state. If None, will use
#                 `state.generated_sql_query`.
#             state (RedboxState): The RedboxState containing the tabular documents.

#         Returns:
#             Tuple[pd.DataFrame, List[Document]]:
#                 - pd.DataFrame: The result of executing the SQL query on the documents.
#                 - List[Document]: The documents that were used in the query execution.
#         """
#         if state is None:
#             raise ValueError("State is required")

#         query_to_execute = sql_query or getattr(state, "generated_sql_query", None)
#         if not query_to_execute:
#             return pd.DataFrame(), []

#         # Extract tabular documents from state
#         documents: DocumentState = getattr(state, table_field, None)
#         if not documents or not documents.groups:
#             return pd.DataFrame(), []

#         used_docs: List[Document] = []
#         con = duckdb.connect(database=":memory:")

#         # Register each tabular doc as a DuckDB table
#         for group_name, group in documents.groups.items():
#             for doc_id, doc in group.items():
#                 if doc is None:
#                     continue
#                 table_name = getattr(doc.metadata, "table_name", f"{group_name}_{doc_id}")
#                 df = pd.DataFrame(doc.page_content)  # assuming doc.page_content is row-like
#                 con.register(table_name, df)
#                 used_docs.append(doc)

#         # Execute SQL query on registered tables
#         try:
#             result_df = con.execute(query_to_execute).df()
#         except Exception as e:
#             raise ValueError(f"Error executing SQL query: {e}")

#         return result_df, used_docs
