...
def build_query_tabular_knowledge_base_tool(
    ...
    def _query_tabular_knowledge_base(
        ...
        for file_meta in tabular_metadata:
            ...
            uri = metadata["uri"]
            schema = metadata["document_schema"]
            ...
            try:
                ...
                if uri.lower().endswith(".csv"):
                    con.execute(f"CREATE TABLE {schema_obj.name} AS SELECT * FROM read_csv_auto('{uri}')")
                elif uri.lower().endswith((".xls", ".xlsx")):
                    con.execute(f"CREATE TABLE {metadata['name']} AS SELECT * FROM read_excel('{uri}')")
                else:
