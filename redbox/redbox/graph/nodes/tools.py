    ...
    def _query_tabular_knowledge_base(
        ...
        try:
            ...
            docs_metadata = retriever._get_relevant_documents(
                permitted_s3_keys=state.request.knowledge_base_s3_keys, uris=[uri], run_manager=None
            )
...
def build_query_tabular_file_tool(
    ...
    def _query_tabular_file(
        ...
        try:
            # Retrieve tabular documents
            docs_metadata = retriever._get_relevant_documents(
                permitted_s3_keys=state.request.permitted_s3_keys, uris=[uri], run_manager=None
            )
            if not docs_metadata:
                return "No documents found for URI", []
            ...
            logger.exception("Unexpected error in _query_tabular_file")
            return f"Unexpected error: {e}", []
