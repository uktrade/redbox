                ...
                async with streamablehttp_client(mcp_url) as (read, write, _):
                    async with ClientSession(read, write) as session:
                        ...
                        if not selected_tool:
                            raise exceptions.ToolNotFoundError(
                        ...
                        try:
                            ...
                            raise exceptions.ToolTimeoutError(tool_name=tool_name, timeout_seconds=0)
                        except Exception as e:
                            raise exceptions.ToolFailedError(tool_name=tool_name, cause=e)

                        log.warning(f"result: {result}")
                        return result

            except (
                exceptions.ToolNotFoundError,
                exceptions.ToolInputValidationError,
                exceptions.ToolTimeoutError,
                exceptions.ToolFailedError,
            ):
                raise  # re-raise typed exceptions as-is
            except asyncio.TimeoutError as e:
                raise exceptions.ToolTimeoutError(tool_name=tool_name, timeout_seconds=tool_timeout) from e
