...
test-redbox: ## Test redbox
    ...
    PYTHONPATH=$(PWD)/django_app:$(PWD)/redbox poetry run pytest tests/test_ingest.py::test_document_loader -m "not ai" --cov=redbox -v --cov-report=term-missing --cov-report=xml --cov-fail-under=75

.PHONY: test-django
test-django: ## Test django-app
	cd django_app && poetry install && poetry run pytest tests/test_consumers.py::test_chat_consumer_agentic --cov=redbox_app -v --cov-report=term-missing --cov-report=xml --cov-fail-under=80 --ds redbox_app.settings $(TEST)
