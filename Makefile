...
test-django: ## Test django-app
	cd django_app && poetry install && poetry run pytest --create-db --no-migrations --cov=redbox_app -v --cov-report=term-missing --cov-report=xml --cov-fail-under=80 --ds redbox_app.settings $(TEST)
