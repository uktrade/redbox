...
test(`Message input functionality`, async ({ page }) => {
  ...
  const messageInput = page.locator(".ids-message-input");

  const height1 = await page.evaluate(
    () => document.querySelector(".ids-message-input").scrollHeight
  );
  ...
  const text1 = await page.evaluate(() =>
    document.querySelector("ids-message-input").getValue()
  );
  expect(text1).toEqual("Test line 1\nTest line 2");
  await page.evaluate(() => document.querySelector("ids-message-input").reset());
